...
For each state update, the physics model is used to recalculate the hole density distribution. While this sounds computationally expensive, the estimation algorithm is optimised through just-in-time (JIT) compilation. Furthermore, the nature of actions that can be taken makes it such that a large portion of a given state does not change/reflect QD formation from one state to the next. For instance, the corners of the SEM image are not relevant for a central QD. This allows for a double optimisation: cropping of the SEM image for state estimation (seen in @initial_layout) and using cached results to populate regions with repeating values. These tremendously speed up the estimation algorithm to just $~$ 2 seconds per state update.
...
To simplify the QD extraction process, a central crop of the hole density map image is considered. This lies vertically between the centre of the uppermost accumulation gate and the middle of the plunger gate, and horizontally between the centres of the barrier gates as shown in @central_crop, bounded by $O_1, O_2, ..., O_5$.

To arrive at a suitable estimation of the QD position and shape as efficiently as possible, the QD extraction algorithm aims to define the QD using its edges, reducing the number of pixels involved in further processing. To do this, the algorithm first performs a thresholding operation on the central crop to filter out non-QD hole densities. This eliminates bulk of the pixels in the central crop, leaving only the pixels that are likely to be constituent to a QD. A histogram of the remaining pixel hole densities is constructed and stored for assessment of the quality of the region of interest.Then, a set of edge pixels (sorted by row, then column) that define the boundary of the region of interest is found. The edge pixels are found by checking each pixel's neighbourhood, where if a pixel is missing a neighbouring pixel in any direction, it is added to the set. As the hole density values are taken care of with the histogram, this set becomes sufficient to define the QD in later processing.
