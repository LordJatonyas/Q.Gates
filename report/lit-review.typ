...
=== Stochastic Games
The stochastic game model was introduced by Shapley to address the need for a general framework that could model dynamic interactions among multiple decision-makers where the state changes in response to the players’s choices @stochastic-games. Stochastic games can be either infinite-horizon (game is played over an infinite number of steps) or finite-horizon (game is played over a finite number of steps) games.
